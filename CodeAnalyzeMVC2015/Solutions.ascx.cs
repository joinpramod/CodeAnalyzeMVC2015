...
public partial class Solutions : System.Web.UI.UserControl
    {
        ...
        protected void PreviewButton_Click(object sender, EventArgs e)
        {
            ...
            if (Session["User"] != null)
            {
                ...
                SolutionEditor.Content = SolutionEditor.Content.Replace("<div>", "");
                SolutionEditor.Content = SolutionEditor.Content.Replace("</div>", "");
                ...
                {
                    if (SolutionEditor.Content.Length > 10000)
                    {
                        SolutionEditor.Content = SolutionEditor.Content.Substring(0, 10000);
                    }

                    replies.Reply = Sanitizer.GetSafeHtml(SolutionEditor.Content);
                }
        ...
        private void BindSolution(string strQuery)
        {
            ConnManager connManager = new ConnManager();
            connManager.OpenConnection();
            ...
            if (dsSolution != null)
            {
                for (int i = 0; i < dsSolution.Tables[0].Rows.Count; i++)
                {
                    ...
                    htcReplyContent.InnerHtml = htcReplyContent.InnerHtml.Replace("#codestart", "<pre class=\"prettyprint\" style=\"font-size:14px;\">");
